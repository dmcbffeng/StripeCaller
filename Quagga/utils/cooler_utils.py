# -*- coding: utf-8 -*-
"""
Adapted from https://github.com/open2c/cooler/blob/master/cooler/util.py
"""
...
def parse_region(reg, chromsizes=None):
    ...
    try:
        key_check = list(chromsizes.keys())[0]
        if key_check.startswith("chr"):
            clen = chromsizes["chr"+chrom] if chromsizes is not None else None
        else:
