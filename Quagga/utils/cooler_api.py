# -*- coding: utf-8 -*-
"""
Adapted from https://github.com/open2c/cooler/blob/master/cooler/api.py
"""
...
from .cooler_utils import parse_cooler_uri, parse_region, open_hdf5, closing_hdf5, list_coolers
...
def dump(cool_uri, resolution, chrloc1, chrloc2, norm):
    ...
    h5 = c.open("r")
    chr_name1 = chrloc1[3:] if chrloc1.startswith('chr') else chrloc1
    i0, i1 = region_to_extent(
        h5, c._chromids, parse_region(chr_name1, c.chromsizes), binsize=c.binsize
    )

    chr_name2 = chrloc2[3:] if chrloc2.startswith('chr') else chrloc2
    j0, j1 = region_to_extent(
        h5, c._chromids, parse_region(chr_name2, c.chromsizes), binsize=c.binsize
    )

    field = "count"
    name = "weight"
    divisive_weights = False
    if norm and name not in h5["bins"]:
        raise ValueError(
            "No column 'bins/{}'".format(name)
            + "found. Use ``cooler.balance_cooler`` to "
            + "calculate balancing weights or set balance=False."
    ...
    for x, y, c in zip(mat.row, mat.col, mat.data):
        if norm:
            yield x, y, c, bias1[x], bias2[y]
        else:
            yield x, y, c, 1.0, 1.0
